...
The project includes Ansible playbooks for automated deployment to Ubuntu 20.04+:

1. **Configure inventory**
Edit `ansible/servers` with your server IP and `ansible/group_vars/all` with your settings.

2. **Initial deployment**
```bash
ansible-playbook -i ansible/servers ansible/djangodeployubuntu20.yml
```

3. **Setup SSL certificates**
```bash
ansible-playbook -i ansible/servers ansible/setup_ssl.yml
```

4. **Update nginx configuration**
```bash
ansible-playbook -i ansible/servers ansible/update_nginx.yml
```

## API Endpoints

### `POST /api/session/`
Creates a new page session with browser info and geolocation.
...
### `POST /api/click/`
Records a button click with elapsed time.
...
### `POST /api/reclick/`
Records reclick attempts (for analytics).

### `GET /api/stats/`
Returns comprehensive global statistics in JSON format.
...
- `browser_name` / `browser_version` - Parsed browser info
- `referrer` - HTTP referrer
- `country_code` / `country_name` - Geographic data
- `clicked` - Boolean flag
- `time_to_click` - Click time in seconds
- `reclick_attempts` - Number of reclick tries
- `loaded_at` - Timestamp

### ButtonClick
- `id` - Auto-incrementing primary key
- `session` - Foreign key to PageSession
- `time_elapsed` - Click time (denormalized for queries)
- `clicked_at` - Timestamp

## Performance Optimizations

- **Database indexing** on frequently queried fields
- **Static asset caching** (35-day expiration)
...
The site uses:
- **Clicky** - Privacy-friendly web analytics with anti-adblock proxy
- **Django ORM** - First-party button click analytics
